...
class NavVC: UINavigationController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate {
                        ...
                        for item in contacts{
                            if let currentUserID = Auth.auth().currentUser?.uid {
                                for number in item.phoneNumbers{
                                    let number = ContactsManager.CNPhoneNumberToString(CNPhoneNumber: number.value)
                                    User.search(forPhoneNumber: number, completion: { (users) in
        ...
        if let id = Auth.auth().currentUser?.uid {
            User.info(forUserID: id, completion: {[weak weakSelf = self] (user) in
                DispatchQueue.main.async {
                    ...
                    weakSelf?.emailLabel.text = user.number
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if self.items.count == 0 {
            ...
            Database.database().reference().child("list").child(self.items[indexPath.row].number).child("status").queryOrderedByValue().queryEqual(toValue: true).observe(.value) { (data: DataSnapshot) in
                    cell.profilePic.layer.borderColor = UIColor.green.cgColor
                }
            Database.database().reference().child("list").child(self.items[indexPath.row].number).child("status").queryOrderedByValue().queryEqual(toValue: false).observe(.value) { (data: DataSnapshot) in
                cell.profilePic.layer.borderColor = UIColor.red.cgColor
            }
